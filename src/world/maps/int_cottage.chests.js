 ...
 *
 * The wardrobe is bolted onto the prop int_cottage.js already draws (`searches`): a second interactable on the
 * same tile made map.nearestInteractable() pick the no-talk furniture forever (P30 gap #6).
 */
import { Treasure } from '../treasure.js';

/** Props the room already stands — searchable in place, no second interactable. */
export const searches = [
  { id: 'ct_wardrobe', name: 'the wardrobe', kind: 'wardrobe',
    text: ['%HERO% opens the wardrobe.{wait:350}{n}Sunday coats, mothballs, and a\nhat with a pheasant feather in\nit that nobody has worn since\nthe wedding.',
      'The door swings shut behind him\non its own.{p}He gets out fast.'] },
];

Treasure.boot({ searches: { int_cottage: searches } });
